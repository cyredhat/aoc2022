...
import re
lines = [re.split('[\\s=;,]+', x) for x in open('day16/input.txt').read().splitlines()]
...
def visit(v, budget, state, flow, answer):
    ...
    max_answer = max(max_answer, flow)
    theo_max = flow
    for u in F:
      theo_max += (budget - T[v][u] - 1) * F[u] if budget - T[v][u] - 1 > 0 else 0
    if theo_max < max_answer:
      return answer
    for u in F:
